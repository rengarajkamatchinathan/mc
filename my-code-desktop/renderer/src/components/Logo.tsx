 ...
 * animates it like a terminal: idle = the halo breathes + the slash pulses
 * like a resting cursor, thinking/tool = the slash tick-rotates like the
 * ASCII spinner (| / - \), streaming = the slash warms up, error = red
 * strobe. Glow and motion live on the `.halo` layer or are opacity-only —
 * the svg itself is never scaled or filtered, which would rasterize it and
 * blur the glyphs. Omit `mood` for a static mark.
 */
...
// Static fallbacks only — styles.css overrides these (the slash follows the
// theme accent via --accent-hover; letters flip to ink when tileless on light).
const SLASH = "#d97757";
const CREAM = "#f0eee6";
...
export function Logo({
  ...
  const svg = (
    <svg
      ...
      aria-label="my-code"
      // styles.css keys off this: tiled marks keep cream letters in light theme
